...
class project(SentoBaseData):
    _logger = makeCustomLogger("project")
    _get_all_filter_meta: dict[str, dict] = {}
    _fields: list[str] = ["id", "name", "owner"]
    _primary_keys: list[str] = ["id"]
    _unique_fields: list[str] = []
    _non_unique_fields: list[str] = ["name", "owner"]
    ...
    _orm: type[ORMproject] = ORMproject

    def __init__(
        self, id: int = None, name: str = None, owner: str = None, *args, **kwargs
    ):
        super().__init__()
    ...
    def owner(self, new_owner):
        if not hasattr(self, "__owner") or new_owner is not None:
            ...
            for k, v in kwargs.items():
                ...
                if v is not None:
                    if filter_meta.get("condition", "==") == "in":
                        v_split = v.split(",")
                        filter = (
                            f"ORMproject.{filter_meta.get('column', '==')}.in_(v_split)"
                        )
                    else:
                        filter = f"ORMproject.{filter_meta.get('column', '==')} {filter_meta.get('condition', '==')} v"
                    filters.append(eval(filter))
            with indexingSession() if db is None else nullcontext(db) as db:
                # items = db.query(ORMproject).filter(
                #     *(getattr(ORMproject, k) == v for k, v in kwargs.items())
                # ).all()
                items = db.query(ORMproject).filter(*filters).limit(limit).all()
    ...
    async def async_get_all(cls, limit=None, adb=None, **kwargs):
        try:
            ...
            async with (
                ...
                #     *(getattr(ORMproject, k) == v for k, v in kwargs.items())
                # ).all()
                stmt = select(ORMproject).filter(*filters).limit(limit)
    ...
    async def async_from_id(cls, id: int, adb=None):
        try:
            async with (
                ...
                data = await adb.get(ORMproject, id)
            if data:
                return cls.from_orm(data)
            else:
                return None
    ...
    def create(self):
        self.save()
        self._logger.debug(f"Created new project with id: {self.id}")
        # try:
    ...
    @classmethod
    def from_orm(cls, orm: ORMproject):
        return cls(
            **{
                field.name: getattr(orm, field.name)
                for field in inspect(ORMproject).c
    ...
    def to_orm(self, db, safe=False):
        ...
        if self.id:
            self_ORM = db.query(ORMproject).with_for_update().get(self.id)
        else:
            del d["id"]
            if safe:
    ...
    async def async_to_orm(self, adb, safe=False):
        d = self.to_create_dict()
        if self.id:
            self_ORM = await adb.get(ORMproject, self.id, with_for_update=True)
        else:
            del d["id"]
            if safe:
                unique_data = {k: v for k, v in d.items() if k in self._unique_fields}
                stmt = select(ORMproject).filter_by(**unique_data).with_for_update()
    ...
    def unsafe_safe_save(self, db=None, safe=False):
        ...
        with indexingSession() if not db else nullcontext(db) as db:
            if self.id:
                # update
                self_ORM = db.get(ORMproject, self.id)
            ...
            if commit:
                self._logger.debug(
                    f"Committing project({self.id}) to db. New: {len(db.new)}, Dirty: {len(db.dirty)}, Deleted: {len(db.deleted)}"
                )
                db.commit()
        return self
    ...
    async def async_unsafe_safe_save(self, adb=None, safe=False):
        ...
        async with indexingAsyncSession() if not adb else nullcontext(adb) as adb:
            if self.id:
                # update
                self_ORM = await adb.get(ORMproject, self.id)
                for key, value in self.to_create_dict().items():
                    setattr(self_ORM, key, value)
            elif len(unique_fields) > 0:
                # check if exists
            ...
            if commit:
                self._logger.debug(
                    f"Committing project({self.id}) to db. New: {len(adb.new)}, Dirty: {len(adb.dirty)}, Deleted: {len(adb.deleted)}"
                )
                await adb.commit()
        return self
    ...
    def save(self, db=None):
        ...
        try:
            ...
            self._logger.warning(
                f"IntegrityError while saving project({self.id}) to db. Trying safe save."
            )
            res = self.unsafe_safe_save(db=db, safe=True)
        self._logger.info(
            f"Saved project({self.id}) to db in {time.time() - start:.3f} seconds"
        )
        return res
    ...
    def delete(self, db=None):
        ...
        if self.__id:
            try:
                with indexingSession() if db is None else nullcontext(db) as db:
                    data = db.get(ORMproject, self.__id)
                    db.delete(data)
                    db.commit()
                self.__id = None
            except Exception as e:
        ...
        if self.__id:
            try:
                async with (
                    ...
                    data = await adb.get(ORMproject, self.__id)
    ...
    def to_update_dict(self):
        return dict(name=self.__name, owner=self.__owner, modified_on=datetime.now())
