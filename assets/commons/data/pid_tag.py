...
from models import pid_tag as ORMpid_tag


class pid_tag(SentoBaseData):
    _logger = makeCustomLogger("pid_tag")
    ...
    _unique_fields: list[str] = ["pid_file_page_id", "name", "type"]
    ...
    _orm: type[ORMpid_tag] = ORMpid_tag
    ...
    @tag_value.setter
    def tag_value(self, new_tag_value):
        if not hasattr(self, "__tag_value") or new_tag_value is not None:
            self.__tag_value = new_tag_value
    ...
    def sub_type(self, new_sub_type):
        if not hasattr(self, "__sub_type") or new_sub_type is not None:
            self.__sub_type = new_sub_type
    ...
    def confidence(self, new_confidence):
        if not hasattr(self, "__confidence") or new_confidence is not None:
    ...
    def candidates(self, new_candidates):
        if not hasattr(self, "__candidates") or new_candidates is not None:
            self.__candidates = new_candidates
            ...
            for k, v in kwargs.items():
                ...
                if v is not None:
                    if filter_meta.get("condition", "==") == "in":
                        v_split = v.split(",")
                        filter = (
                            f"ORMpid_tag.{filter_meta.get('column', '==')}.in_(v_split)"
                        )
                    else:
                        filter = f"ORMpid_tag.{filter_meta.get('column', '==')} {filter_meta.get('condition', '==')} v"
                    filters.append(eval(filter))
            with indexingSession() if db is None else nullcontext(db) as db:
                # items = db.query(ORMpid_tag).filter(
                #     *(getattr(ORMpid_tag, k) == v for k, v in kwargs.items())
                # ).all()
                items = db.query(ORMpid_tag).filter(*filters).limit(limit).all()
    ...
    async def async_get_all(cls, limit=None, adb=None, **kwargs):
        try:
            ...
            async with (
                ...
                #     *(getattr(ORMpid_tag, k) == v for k, v in kwargs.items())
                # ).all()
                stmt = select(ORMpid_tag).filter(*filters).limit(limit)
    ...
    async def async_from_id(cls, id: int, adb=None):
        try:
            async with (
                ...
                data = await adb.get(ORMpid_tag, id)
            if data:
                return cls.from_orm(data)
            else:
                return None
    ...
    def from_dict(cls, new_obj: Dict):
        if not {"pid_file_page_id", "name", "type"}.issubset(new_obj.keys()):
            raise KeyError("dict should contain at least pid_file_page_id,name,type")
        return cls(**new_obj)

    def create(self):
        self.save()
        self._logger.debug(f"Created new pid_tag with id: {self.id}")
        # try:
    ...
    @classmethod
    def from_orm(cls, orm: ORMpid_tag):
        return cls(
            **{
                field.name: getattr(orm, field.name)
                for field in inspect(ORMpid_tag).c
    ...
    def to_orm(self, db, safe=False):
        ...
        if self.id:
            self_ORM = db.query(ORMpid_tag).with_for_update().get(self.id)
        else:
            del d["id"]
            if safe:
    ...
    async def async_to_orm(self, adb, safe=False):
        d = self.to_create_dict()
        if self.id:
            self_ORM = await adb.get(ORMpid_tag, self.id, with_for_update=True)
        else:
            del d["id"]
            if safe:
                unique_data = {k: v for k, v in d.items() if k in self._unique_fields}
                stmt = select(ORMpid_tag).filter_by(**unique_data).with_for_update()
    ...
    def unsafe_safe_save(self, db=None, safe=False):
        ...
        with indexingSession() if not db else nullcontext(db) as db:
            if self.id:
                # update
                self_ORM = db.get(ORMpid_tag, self.id)
            ...
            if commit:
                self._logger.debug(
                    f"Committing pid_tag({self.id}) to db. New: {len(db.new)}, Dirty: {len(db.dirty)}, Deleted: {len(db.deleted)}"
                )
                db.commit()
        return self
    ...
    async def async_unsafe_safe_save(self, adb=None, safe=False):
        ...
        async with indexingAsyncSession() if not adb else nullcontext(adb) as adb:
            if self.id:
                # update
                self_ORM = await adb.get(ORMpid_tag, self.id)
                for key, value in self.to_create_dict().items():
                    setattr(self_ORM, key, value)
            elif len(unique_fields) > 0:
                # check if exists
            ...
            if commit:
                self._logger.debug(
                    f"Committing pid_tag({self.id}) to db. New: {len(adb.new)}, Dirty: {len(adb.dirty)}, Deleted: {len(adb.deleted)}"
                )
                await adb.commit()
        return self
    ...
    def save(self, db=None):
        ...
        try:
            ...
            self._logger.warning(
                f"IntegrityError while saving pid_tag({self.id}) to db. Trying safe save."
            )
            res = self.unsafe_safe_save(db=db, safe=True)
        self._logger.info(
            f"Saved pid_tag({self.id}) to db in {time.time() - start:.3f} seconds"
        )
        return res
    ...
    def delete(self, db=None):
        ...
        if self.__id:
            try:
                with indexingSession() if db is None else nullcontext(db) as db:
                    data = db.get(ORMpid_tag, self.__id)
                    db.delete(data)
                    db.commit()
                self.__id = None
            except Exception as e:
        ...
        if self.__id:
            try:
                async with (
                    ...
                    data = await adb.get(ORMpid_tag, self.__id)
